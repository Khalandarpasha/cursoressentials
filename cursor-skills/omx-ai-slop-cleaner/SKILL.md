...
description: >-
  Run an anti-slop cleanup/refactor/deslop workflow that preserves behavior
  and raises signal quality via regression-tests-first, smell-by-smell passes.
---
# AI Slop Cleaner (ported from OmX $ai-slop-cleaner)
...
- The code is already clean and well-tested
- The user wants a full rewrite or architectural change
- The task is a simple single-file fix
...
- For mandatory deslop passes on recently changed files, run this skill on the changed files only in standard mode unless the caller explicitly requests otherwise.
...
4. **Execute passes one smell at a time**
   ...
   - Re-run targeted verification after each pass (use `ReadLints tool` for lint checks)
   - Avoid bundling unrelated refactors into the same edit set

5. **Run quality gates**
   - Regression tests stay green
   - Lint passes (use `ReadLints tool`)
   - Typecheck passes (use `Shell tool` to run type checker)
...
- **Shell tool** — run build, test, typecheck, lint commands
- **ReadLints tool** — check for linter errors after edits
- **StrReplace tool** — make targeted code edits
- **Grep tool** — search for patterns, dead code, duplications
- **Task tool with explore subagent** — find files and understand codebase structure
